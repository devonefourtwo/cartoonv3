...
export default function Home() {
  ...
  async function generate() {
    ...
    try {
      ...
      // Go straight to auto-download — no preview shown first
    ...
    <div className="min-h-screen bg-gradient-to-br from-indigo-950 via-purple-950 to-black">
      <div className="fixed inset-0 overflow-hidden pointer-events-none">
        {Array.from({length:60}).map((_,i)=>(
          <div key={i} className="absolute rounded-full bg-white"
            style={{width:i%7===0?3:2,height:i%7===0?3:2,top:`${(i*37)%100}%`,left:`${(i*61+13)%100}%`,opacity:0.15+((i%5)*0.08)}}/>
        ))}
      </div>
      <div className="relative max-w-3xl mx-auto px-5 py-16">
        <div className="text-center mb-12">
          ...
          <p className="text-white/60 text-lg">Type a story idea → get a full 5-minute animated cartoon</p>
          <div className="flex flex-wrap items-center justify-center gap-4 mt-4 text-sm text-white/40">
            <span>✓ 30 scenes</span><span>✓ Voice acting</span>
            <span>✓ Background music</span><span>✓ Auto-downloads</span>
          </div>
        </div>
